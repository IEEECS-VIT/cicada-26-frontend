...
export function useAdminDashboard() {
  ...
  const [newChallengeRoundId, setNewChallengeRoundId] = useState('');
  ...
  const buildChallengePayload = (challenge, overrides = {}) => {
    ...
    const orderNum = parseInt(overrides.order_number ?? raw.order_number ?? challenge?.order_number ?? challenge?.round ?? 1, 10);
      ...
      if (Array.isArray(ch?.data) && ch.data.length > 0) {
        ...
        setChallenges(ch.data.map((x) => {
          ...
          return {
            id: x.id,
            title: x.name,
            round: x.round_order ?? x.order_number,
            roundName: x.round_name || null,
            order_number: x.order_number,
            answer: displayAns,
            rawAnswer: rawAns,
  ...
  const handleCreateChallenge = async (e) => {
    ...
    const targetRound = rounds.find((r) => r.id === newChallengeRoundId) || rounds[0] || null;
    ...
    // Auto-assign the next order_number within the selected round
    const roundChallenges = challenges.filter((c) => {
      if (c.raw?.round_id) return targetRound ? c.raw.round_id === targetRound.id : false;
      return targetRound ? (c.round || 1) === (targetRound.order_number || 1) : false;
    });
    const orderNum = roundChallenges.reduce((max, c) => Math.max(max, c.order_number || c.round || 0), 0) + 1;
    ...
    try {
      ...
      if (targetRound?.id && isUUID(targetRound.id)) {
        payload.round_id = targetRound.id;
      }
