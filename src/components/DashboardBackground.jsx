...
export default function DashboardBackground() {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-[url('/assets/dashboard-nebula.png')] bg-cover bg-[68%_34%] bg-no-repeat z-0 pointer-events-none" />
      <div className="fixed inset-0 bg-[linear-gradient(180deg,rgba(7,5,10,.82)_0%,rgba(7,5,10,.45)_14%,transparent_30%),linear-gradient(115deg,rgba(7,5,10,.78)_0%,rgba(9,6,8,.5)_42%,rgba(12,8,7,.34)_70%,rgba(7,5,9,.72)_100%)] z-0 pointer-events-none" />
      <canvas ref={skyRef} className="fixed inset-0 w-full h-full block z-[1] pointer-events-none" />
      <div className="fixed rounded-full z-0 pointer-events-none -right-[14vw] top-[8vh] w-[58vw] h-[58vw] bg-[radial-gradient(circle_at_50%_50%,rgba(232,168,116,.2)_0%,rgba(178,104,58,.1)_34%,rgba(90,44,20,.04)_58%,transparent_72%)] blur-[18px] animate-[cd-glow_14s_ease-in-out_infinite]" />
      <div className="fixed rounded-full z-0 pointer-events-none -left-[22vw] -bottom-[18vh] w-[56vw] h-[56vw] bg-[radial-gradient(circle_at_50%_50%,rgba(140,150,190,.1)_0%,rgba(60,70,110,.05)_45%,transparent_70%)] blur-[26px] animate-[cd-glow_19s_ease-in-out_infinite]" />
      <div className="fixed inset-0 bg-[radial-gradient(115%_85%_at_50%_45%,transparent_45%,rgba(4,3,6,.55)_78%,rgba(3,2,5,.9)_100%)] z-[1] pointer-events-none" />
    </>
  );
}
